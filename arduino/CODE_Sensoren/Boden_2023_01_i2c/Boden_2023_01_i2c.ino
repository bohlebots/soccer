...
void setup()
{
  ...
  //#if CONFIG_IDF_TARGET_ESP32 char message[64]; snprintf(message, 64, "%u Packets.", i++); Wire.slaveWrite((uint8_t *)message, strlen(message));
}
...
void loop()
{
  ...
  linie_erkennen();
  pakete_erstellen();
  //nicht mehr Nötig, weil i2c per Interrupt gesteuert wird.
  //hoffentlich auf core1?
  //checkCanBus();
  //schreibe();
  //Serial.println(" Linie :" + String(weiss_abweichung));
  while (zeit < 2) delay(1); //checkCanBus();    // fülle auf 1 Sekunde auf - nur Diagnose !!!(momentan 2 milisekunden)
